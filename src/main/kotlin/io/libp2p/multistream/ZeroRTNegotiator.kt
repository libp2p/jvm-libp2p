...
import io.libp2p.etc.util.netty.StringSuffixDecoder
import io.libp2p.etc.util.netty.StringSuffixEncoder
...
object ZeroRTNegotiator {
    ...
    abstract class GenericHandler : SimpleChannelInboundHandler<String>() {
        ...
        val outPrehandlers = listOf(
            ProtobufVarint32LengthFieldPrepender(),
            StringEncoder(Charsets.UTF_8),
            StringSuffixEncoder('\n')
        )

        val inPrehandlers = listOf(
        ...
        val prehandlers = outPrehandlers + inPrehandlers

        var headerRead = false
    }

    class RequesterHandler(val protocol: String) : GenericHandler() {
        var i = 0

        override fun channelActive(ctx: ChannelHandlerContext) {
            ctx.write(MULTISTREAM_PROTO)
            ctx.write(protocol)
            outPrehandlers.forEach { ctx.pipeline().remove(it) }
            ctx.fireUserEventTriggered(ProtocolNegotiationSucceeded(protocol))
        ...
        override fun channelRead0(ctx: ChannelHandlerContext, msg: String) {
            if (msg == MULTISTREAM_PROTO) {
                ...
                if (msg != protocol) {
                    ctx.pipeline().fireUserEventTriggered(ProtocolNegotiationFailed(listOf(protocol)))
                }
                ctx.pipeline().remove(this@RequesterHandler)
                // DelimiterBasedFrameDecoder should be removed last since it
                // propagates unhandled bytes on removal
                inPrehandlers.reversed().forEach { ctx.pipeline().remove(it) }
            }
        }
    }
    ...
    class ResponderHandler(val protocols: List<ProtocolMatcher>) : GenericHandler() {
        ...
        override fun channelRead0(ctx: ChannelHandlerContext, msg: String) {
            if (msg == MULTISTREAM_PROTO) {
                ...
                processMsg(ctx, msg)?.also { completeEvent ->
                    ...
                    ctx.pipeline().remove(this@ResponderHandler)
                    // DelimiterBasedFrameDecoder should be removed last since it
                    // propagates unhandled bytes on removal
        ...
        fun processMsg(ctx: ChannelHandlerContext, msg: String): Any? {
            return when {
